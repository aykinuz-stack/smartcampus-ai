"""
Fono Almanca - Kendi Kendine Almanca Egitim Portali
46 Ders, ders ders ilerleyen, interaktif alistirmali ogrenme modulu
"""
...
# Sabitler
# ---------------------------------------------------------------------------
_DATA_DIR = pathlib.Path(__file__).resolve().parent.parent / "data" / "fono_almanca"
_LESSONS_FILE = _DATA_DIR / "fono_almanca_lessons.json"
_PROGRESS_FILE = _DATA_DIR / "user_progress.json"
_GUNLUK_FILE = _DATA_DIR / "gunluk_almanca.json"
_PREFIX = "fono_de_"

_BOOK_INFO = {
    1: {"title": "Temel Ifadeler ve Okunus", "level": "A1 Baslangic", "range": (1, 12),
        "color": "#16a34a", "color_light": "#4ade80", "color_bg": "#dcfce7", "emoji": "\U0001f4d7"},
    2: {"title": "Okunus ve Telaffuz Kurallari", "level": "A1-A2 Telaffuz", "range": (13, 24),
        "color": "#2563eb", "color_light": "#60a5fa", "color_bg": "#dbeafe", "emoji": "\U0001f4d8"},
    3: {"title": "Yolculuk ve Konaklama", "level": "A2 Temel", "range": (25, 34),
        "color": "#7c3aed", "color_light": "#a78bfa", "color_bg": "#ede9fe", "emoji": "\U0001f4d9"},
    4: {"title": "Gunluk Yasam ve Is", "level": "A2-B1 Orta", "range": (35, 46),
...
def _load_gunluk() -> dict:
    if _GUNLUK_FILE.exists():
        with open(_GUNLUK_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {}
...
def _render_tts_inline(text: str, key: str) -> str:
    # Legacy: superseded by component-based TTS (_render_tts_button / _render_vocab_tts_grid)
    """Return empty string - inline TTS not used, we use component-based TTS."""
    return ""
...
def _render_header():
    st.markdown("""
    <div class="fono-header" style="background:linear-gradient(135deg,#064e3b 0%,#16a34a 50%,#15803d 100%);box-shadow:0 8px 32px rgba(22,163,74,0.3);">
        <h1>FONO Kendi Kendine Almanca</h1>
        <p>46 Ders | 4 Kitap | Okunus Kurallari + Konusma Kilavuzu</p>
    </div>
    """, unsafe_allow_html=True)
...
def _render_stats(lessons: list, progress: dict):
    completed = set(progress.get("completed_lessons", []))
    total = len(lessons) if lessons else 46
    done = len(completed)
...
# GUNLUK ALMANCA
# ---------------------------------------------------------------------------
def _render_gunluk_almanca():
    data = _load_gunluk()
    if not data or not data.get("chapters"):
        st.warning("Gunluk Almanca verileri yuklenemedi.")
        return
    ...
    sel_ch_label = st.selectbox("Bolum Secin:", ch_options, key=f"{_PREFIX}gunluk_ch")
    ...
    sel_sec_label = st.selectbox("Alt Konu:", sec_options, key=f"{_PREFIX}gunluk_sec_{chapter['id']}")
    ...
    view_mode = st.radio("Gorunum:", ["Kartlar", "Liste", "Quiz"],
                         key=f"{_PREFIX}gunluk_mode_{chapter['id']}_{section['id']}",
                         horizontal=True, label_visibility="collapsed")

    if view_mode == "Kartlar":
        _render_gunluk_cards(phrases, section['id'])
    elif view_mode == "Liste":
        _render_gunluk_list(phrases, section['id'])
    elif view_mode == "Quiz":
        _render_gunluk_quiz(phrases, section['id'])


def _render_gunluk_cards(phrases: list, sec_id: str):
...
def _render_gunluk_list(phrases: list, sec_id: str):
...
def _render_gunluk_quiz(phrases: list, sec_id: str):
    if len(phrases) < 3:
        st.info("Quiz icin en az 3 ifade gerekli.")
        return

    quiz_key = f"{_PREFIX}gunluk_quiz_{sec_id}"
...
def render_fono_almanca():
    ...
    if view == "lesson":
        ...
        tabs = st.tabs(["Dersler", "Ilerleme", "Kelime Bankasi", "Gunluk Almanca"])
        with tabs[0]:
        ...
        with tabs[3]:
            _render_gunluk_almanca()


# Deprecated — superseded by component TTS (_render_tts_button / _render_vocab_tts_grid)
_DEPRECATED = [_render_tts_inline]
