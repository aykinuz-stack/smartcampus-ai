"""
Fono Ispanyolca - Kendi Kendine Hizli Ispanyolca Egitim Portali
...
# Sabitler
# ---------------------------------------------------------------------------
_DATA_DIR = pathlib.Path(__file__).resolve().parent.parent / "data" / "fono_ispanyolca"
_LESSONS_FILE = _DATA_DIR / "fono_ispanyolca_lessons.json"
_PROGRESS_FILE = _DATA_DIR / "user_progress_es.json"
_PRATIK_FILE = _DATA_DIR / "gunluk_pratik_es.json"
_PREFIX = "fono_es_"

_BOOK_INFO = {
    1: {"title": "Birinci Basamak - Kitap 1", "level": "A1 Baslangic", "range": (1, 26),
        "color": "#ea580c", "color_light": "#fb923c", "color_bg": "#fff7ed", "emoji": "\U0001f4d9"},
    2: {"title": "Birinci Basamak - Kitap 2", "level": "A1 Baslangic", "range": (27, 52),
        "color": "#ea580c", "color_light": "#fb923c", "color_bg": "#fff7ed", "emoji": "\U0001f4d7"},
    3: {"title": "Ikinci Basamak - Kitap 1", "level": "A2 Temel", "range": (53, 78),
        "color": "#d97706", "color_light": "#fbbf24", "color_bg": "#fef3c7", "emoji": "\U0001f4d8"},
    4: {"title": "Ikinci Basamak - Kitap 2", "level": "A2 Temel", "range": (79, 104),
        "color": "#d97706", "color_light": "#fbbf24", "color_bg": "#fef3c7", "emoji": "\U0001f4d5"},
}
...
function speakES(text, rate) {
    ...
    var pref = ['Monica','Jorge','Paulina','Diego','Laura','Lucia'];
    var esV = null;
    for (var i = 0; i < voices.length; i++) {
        if (voices[i].lang && voices[i].lang.startsWith('es')) {
            if (!esV) esV = voices[i];
            for (var j = 0; j < pref.length; j++) {
                if (voices[i].name.indexOf(pref[j]) !== -1) { esV = voices[i]; break; }
            }
        }
    }
    if (esV) utter.voice = esV;
    synth.speak(utter);
}
...
def _inject_css():
    ...
    .fono-lesson-card {
        ...
        border-left: 4px solid #ea580c; transition: all 0.2s ease;
    }
    ...
    .fono-vocab-es { font-weight: 700; color: #c2410c; font-size: 1.1rem; }
    .fono-vocab-pron { color: #ea580c; font-size: 0.85rem; font-style: italic; }
    ...
    .fono-stat-num { font-size: 1.8rem; font-weight: 800; color: #ea580c; }
...
def _render_header():
    ...
    <div class="fono-header">
        <h1>FONO Kendi Kendine Hizli Ispanyolca</h1>
        <p>104 Ders | 4 Kitap | Adim Adim Ispanyolca Ogrenme Portali</p>
    </div>
    """, unsafe_allow_html=True)
...
def _render_stats(lessons: list, progress: dict):
    ...
    cols = st.columns(4)
    stats = [
        ("Toplam Ders", str(total), "#ea580c"),
        ("Tamamlanan", str(done), "#059669"),
        ("Kalan", str(total - done), "#d97706"),
        ("Ilerleme", f"%{pct}", "#c2410c"),
    ]
...
def _render_vocabulary(lesson: dict, ders_no: int, color: str, color_light: str):
    ...
    if view_mode == "Kartlar":
        # TTS destekli HTML kart gridi
        cards_html = _TTS_JS_CORE + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">'

        for i, w in enumerate(vocab):
            es = w.get("es", "")
            pron = w.get("pron", "")
            tr = w.get("tr", "")
            es_escaped = es.replace("'", "\\'").replace('"', '\\"')

            cards_html += f"""
            <div style="background:linear-gradient(135deg,#111827,#1A2035);padding:12px 16px;
                 border-radius:10px;border:1px solid #e2e8f0;transition:all 0.2s;">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                    <div style="flex:1;">
                        <div style="font-weight:700;color:#c2410c;font-size:1.1rem">{es}</div>
                        <div style="color:#ea580c;font-size:0.85rem;font-style:italic">[{pron}]</div>
                        <div style="color:#475569;font-size:0.95rem;margin-top:2px">{tr}</div>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:4px;margin-left:8px;">
                        <button onclick="speakES('{es_escaped}', 0.85)"
                        ...
                        <button onclick="speakES('{es_escaped}', 0.55)"
...
def _render_flashcard_tts(vocab: list, ders_no: int, color: str, color_light: str):
    ...
    pron = w.get("pron", "")
    tr_word = w.get("tr", "")
    es_escaped = es_word.replace("'", "\\'").replace('"', '\\"')

    meaning_html = f'<div class="meaning">{tr_word}</div>' if show_meaning else '<div class="meaning" style="opacity:0.5">Anlami gormek icin tiklayin</div>'
    ...
    <div style="background:linear-gradient(135deg,{color},{color_light});
        ...
        <div style="font-size:2rem;font-weight:800">{es_word}</div>
        ...
        <div style="display:flex;gap:10px;margin-top:12px;">
            <button onclick="speakES('{es_escaped}', 0.85)"
                ...
                border-radius:24px;padding:6px 16px;cursor:pointer;font-size:0.9rem;font-weight:600;">
                &#128266; Normal</button>
            <button onclick="speakES('{es_escaped}', 0.55)"
        ...
        <div style="font-size:1.2rem;margin-top:12px;opacity:0.9">
            {'<span>' + tr_word + '</span>' if show_meaning else '<span style="opacity:0.5">Anlami gormek icin tiklayin</span>'}
        </div>
...
def _render_grammar(lesson: dict, ders_no: int):
    ...
    if examples:
        ...
        # TTS destekli ornek cumleler
        examples_html = _TTS_JS_CORE + '<div style="display:flex;flex-direction:column;gap:6px;">'
        ...
        for ex in examples:
            ...
            <div style="background:white;border:1px solid #e2e8f0;border-radius:8px;
                 padding:10px 14px;border-left:3px solid #ea580c;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div style="flex:1;">
                        <div style="font-weight:600;color:#c2410c">{es}</div>
                    ...
                    <button onclick="speakES('{es_escaped}', 0.85)"
                        ...
                        box-shadow:0 2px 8px rgba(234,88,12,0.3);flex-shrink:0;margin-left:10px;"
                        title="Sesli Oku">&#128266;</button>
                </div>
            </div>
            """
...
def _render_reading(lesson: dict, ders_no: int):
    ...
    text_es = reading.get("es", "")
    text_tr = reading.get("tr", "")
    es_escaped = text_es.replace("'", "\\'").replace('"', '\\"').replace("\n", " ")

    st.markdown(f"### {title}")

    # TTS destekli okuma parcasi
    ...
    <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:12px;
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
            ...
            <button onclick="speakES('{es_escaped}', 0.8)"
                style="background:linear-gradient(135deg,#ea580c,#c2410c);color:white;
                ...
                box-shadow:0 2px 8px rgba(234,88,12,0.3);flex-shrink:0;"
                title="Metni Sesli Oku">&#128266; Dinle</button>
        </div>
        <div style="color:#78350f;line-height:1.8;white-space:pre-line">{text_es}</div>
    </div>
    """
    height = max(200, len(text_es) // 2 + 100)
...
def _render_exercises(lesson: dict, ders_no: int, progress: dict):
    ...
    for ex_idx, exercise in enumerate(exercises):
        ...
        <div class="fono-exercise-box">
            <div style="font-weight:700; color:#c2410c; font-size:1rem; margin-bottom:4px">
                {ex_title}
            </div>
            <div style="color:#ea580c; font-size:0.85rem">
        ...
        for q_idx, q in enumerate(questions):
            ...
            elif ex_type == "translate_to_es":
                correct = _render_translate(q, q_key, is_checked, "es")
...
def _get_exercise_type_label(ex_type: str) -> str:
    labels = {
        ...
        "translate_to_tr": "Ispanyolca -> Turkce Ceviri",
        "translate_to_es": "Turkce -> Ispanyolca Ceviri",
...
def _render_translate(q: dict, key: str, checked: bool, target: str) -> bool:
    source = q.get("source", "")
    answer = q.get("answer", "")
    direction = "Turkce'ye cevirin" if target == "tr" else "Ispanyolca'ya cevirin"
...
def _check_answer(q: dict, key: str, ex_type: str) -> bool:
    ...
    elif ex_type in ("translate_to_tr", "translate_to_es"):
        valid = [q.get("answer", "")]
        if q.get("alt_answers"):
...
def _render_gunluk_pratik():
    ...
    if not data or not data.get("chapters"):
        st.warning("Gunluk Pratik Ispanyolca verileri yuklenemedi.")
        return

    chapters = data["chapters"]

    st.markdown("""
    <div style="background:linear-gradient(135deg,#7c2d12 0%,#ea580c 50%,#d97706 100%);
         padding:24px 28px;border-radius:14px;margin-bottom:20px;color:white;">
        <h2 style="margin:0;font-size:1.5rem;font-weight:800">Gunluk Pratik Ispanyolca</h2>
        <p style="margin:4px 0 0 0;opacity:0.85;font-size:0.88rem">
            Pratik Ispanyolca-Turkce El Kitabi -- 13 Bolum, 70+ Alt Konu, 500+ Cumle & Sesli Okuma
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#fff7ed,#fed7aa);border:1px solid #fdba74;
         border-radius:12px;padding:14px 18px;margin-bottom:16px;">
        <div style="font-weight:700;color:#c2410c;font-size:1.1rem">{section['title']}</div>
        <div style="color:#ea580c;font-size:0.82rem;margin-top:2px">{len(phrases)} ifade</div>
    </div>
    """, unsafe_allow_html=True)
...
def _render_pratik_cards(phrases: list, sec_id: str):
    ...
    for i, p in enumerate(phrases):
        ...
        es_text = p.get("es", "")
        pron = p.get("pron", "")
        es_escaped = es_text.replace("'", "\\'").replace('"', '\\"')

        cards_html += f"""
        <div style="background:linear-gradient(135deg,#111827,#1A2035);padding:14px 18px;
             border-radius:12px;border:1px solid #e2e8f0;border-left:4px solid #ea580c;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <div style="flex:1;">
                    <div style="font-weight:700;color:#c2410c;font-size:1.05rem;margin-bottom:3px">
                        {es_text}
                    </div>
                    <div style="color:#ea580c;font-size:0.82rem;font-style:italic;margin-bottom:4px">
                        /{pron}/
                    </div>
                ...
                <div style="display:flex;flex-direction:column;gap:4px;margin-left:10px;">
                    ...
                    <button onclick="speakES('{es_escaped}', 0.55)"
                        ...
                        box-shadow:0 2px 8px rgba(217,119,6,0.3);flex-shrink:0;"
                        title="Yavas Oku">&#128034;</button>
                </div>
            </div>
        </div>
...
def _render_pratik_list(phrases: list, sec_id: str):
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
        <thead>
            <tr style="background:linear-gradient(135deg,#7c2d12,#ea580c);color:white;">
                <th style="padding:10px 12px;text-align:left;border-radius:8px 0 0 0;">Turkce</th>
                <th style="padding:10px 12px;text-align:left;">Espanol</th>
    ...
    for i, p in enumerate(phrases):
        ...
        <tr style="background:{bg};border-bottom:1px solid #e2e8f0;">
            ...
            <td style="padding:8px 12px;color:#c2410c;font-weight:700">{es_text}</td>
            <td style="padding:8px 12px;color:#ea580c;font-style:italic;font-size:0.82rem">/{pron}/</td>
            <td style="padding:8px 8px;text-align:center">
                <button onclick="speakES('{es_escaped}', 0.85)"
                    style="background:#ea580c;color:white;border:none;border-radius:50%;
...
def _render_pratik_quiz(phrases: list, sec_id: str):
    """Mini quiz - Turkce'den Ispanyolca'ya ceviri."""
    if len(phrases) < 3:
    ...
    st.markdown("### Mini Quiz -- Turkce -> Ispanyolca")

    correct_count = 0
    for qi, idx in enumerate(indices):
        p = phrases[idx]
        st.markdown(f"**{qi+1}.** {p['tr']}")
        user_ans = st.text_input("Ispanyolca karsiligi:", key=f"{quiz_key}_{qi}",
                                  placeholder="Cevabinizi yazin...", label_visibility="collapsed")
        if is_checked:
            norm_user = user_ans.strip().lower().rstrip(".!?,;:").strip()
            norm_answer = p['es'].strip().lower().rstrip(".!?,;:").strip()
            if norm_user == norm_answer:
                st.success(f"Dogru! {p['es']}")
                correct_count += 1
            else:
                st.error(f"Dogru cevap: **{p['es']}** /{p.get('pron', '')}/")
...
def _render_vocab_bank(lessons: list):
    ...
    search = st.text_input("Kelime Ara:", key=f"{_PREFIX}vb_search", placeholder="Ispanyolca veya Turkce...")
    if search:
        all_words = [(w, d) for w, d in all_words
                     if search.lower() in w.get("es", "").lower()
    ...
    if all_words:
        bank_html = _TTS_JS_CORE + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">'

        for i, (w, ders) in enumerate(all_words):
            es = w.get('es', '')
            pron = w.get('pron', '')
            tr = w.get('tr', '')
            es_escaped = es.replace("'", "\\'").replace('"', '\\"')
            ...
            <div style="background:linear-gradient(135deg,#111827,#1A2035);padding:12px 16px;
                ...
                <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                    ...
                    <button onclick="speakES('{es_escaped}', 0.85)"
                        ...
                        border:none;border-radius:50%;width:32px;height:32px;cursor:pointer;
                        font-size:0.9rem;display:flex;align-items:center;justify-content:center;
                        box-shadow:0 2px 8px rgba(234,88,12,0.3);flex-shrink:0;margin-left:8px;"
                        title="Sesli Oku">&#128266;</button>
                </div>
            </div>
            """
...
def render_fono_ispanyolca():
    ...
    if view == "lesson":
        ...
        tabs = st.tabs(["Dersler", "Ilerleme", "Kelime Bankasi", "Gunluk Pratik Ispanyolca"])
        with tabs[0]:
