"""
Fono Italyanca - Kendi Kendine Hizli Italyanca Egitim Portali
...
# Sabitler
# ---------------------------------------------------------------------------
_DATA_DIR = pathlib.Path(__file__).resolve().parent.parent / "data" / "fono_italyanca"
_LESSONS_FILE = _DATA_DIR / "fono_italyanca_lessons.json"
_PROGRESS_FILE = _DATA_DIR / "user_progress_it.json"
_PRATIK_FILE = _DATA_DIR / "gunluk_pratik_it.json"
_PREFIX = "fono_it_"

_BOOK_INFO = {
    1: {"title": "Birinci Basamak - Kitap 1", "level": "A1 Baslangic", "range": (1, 26),
        "color": "#dc2626", "color_light": "#f87171", "color_bg": "#fee2e2", "emoji": "\U0001f4d5"},
    2: {"title": "Birinci Basamak - Kitap 2", "level": "A1 Baslangic", "range": (27, 52),
        "color": "#dc2626", "color_light": "#f87171", "color_bg": "#fee2e2", "emoji": "\U0001f4d7"},
    3: {"title": "Ikinci Basamak - Kitap 1", "level": "A2 Temel", "range": (53, 78),
        "color": "#be123c", "color_light": "#fb7185", "color_bg": "#ffe4e6", "emoji": "\U0001f4d8"},
    4: {"title": "Ikinci Basamak - Kitap 2", "level": "A2 Temel", "range": (79, 104),
        "color": "#be123c", "color_light": "#fb7185", "color_bg": "#ffe4e6", "emoji": "\U0001f4d9"},
}
...
def _inject_css():
    ...
    .fono-lesson-card {
        ...
        border-left: 4px solid #dc2626; transition: all 0.2s ease;
    }
    ...
    .fono-vocab-it { font-weight: 700; color: #b91c1c; font-size: 1.1rem; }
    .fono-vocab-pron { color: #e11d48; font-size: 0.85rem; font-style: italic; }
    ...
    .fono-exercise-box {
        ...
        border: 1px solid #fda4af; border-radius: 12px;
        padding: 16px 20px; margin: 12px 0;
    }
    ...
    .fono-stat-num { font-size: 1.8rem; font-weight: 800; color: #dc2626; }
...
function speakIT(text, rate) {
    ...
    var pref = ['Alice','Luca','Elsa','Federica','Paola','Giorgio'];
    var itV = null;
    for (var i = 0; i < voices.length; i++) {
        if (voices[i].lang && voices[i].lang.startsWith('it')) {
            if (!itV) itV = voices[i];
            for (var j = 0; j < pref.length; j++) {
                if (voices[i].name.indexOf(pref[j]) !== -1) { itV = voices[i]; break; }
            }
        }
    }
    if (itV) utter.voice = itV;
    synth.speak(utter);
}
...
def _render_header():
    ...
    <div class="fono-header">
        <h1>FONO Kendi Kendine Hizli Italyanca</h1>
        <p>104 Ders | 4 Kitap | Adim Adim Italyanca Ogrenme Portali</p>
    </div>
    """, unsafe_allow_html=True)
...
def _render_vocabulary(lesson: dict, ders_no: int, color: str, color_light: str):
    ...
    if view_mode == "Kartlar":
        _render_vocab_cards_tts(vocab, ders_no, color, color_light)

    elif view_mode == "Flashcard":
        _render_flashcard_tts(vocab, ders_no, color, color_light)


def _render_vocab_cards_tts(vocab: list, ders_no: int, color: str, color_light: str):
    """Kart gorunumu - TTS destekli Italyanca kelime kartlari."""
    ...
    for i in range(0, len(vocab), 3):
        ...
        for j in range(3):
            ...
            pron = w.get("pron", "")
            tr_text = w.get("tr", "")
            it_escaped = it_text.replace("'", "\\'").replace('"', '\\"')

            cards_html += f"""
            <div style="flex:1;min-width:200px;background:linear-gradient(135deg,#111827,#1A2035);
                 padding:14px 16px;border-radius:10px;border:1px solid #fecaca;
                 border-left:4px solid {color};">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                    <div style="flex:1;">
                        <div style="font-weight:700;color:#b91c1c;font-size:1.1rem">{it_text}</div>
                        <div style="color:#e11d48;font-size:0.82rem;font-style:italic">/{pron}/</div>
                        <div style="color:#475569;font-size:0.92rem;margin-top:3px">{tr_text}</div>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:4px;margin-left:8px;">
                        <button onclick="speakIT('{it_escaped}', 0.85)"
                            style="background:linear-gradient(135deg,{color},{color_light});color:white;
                            border:none;border-radius:50%;width:32px;height:32px;cursor:pointer;
                            font-size:0.9rem;display:flex;align-items:center;justify-content:center;
                            box-shadow:0 2px 6px rgba(220,38,38,0.3);"
                            title="Sesli Oku">&#128266;</button>
                        <button onclick="speakIT('{it_escaped}', 0.55)"
    ...
    cards_html += '</div>'
    row_count = (len(vocab) + 2) // 3
    height = row_count * 110 + 60
...
def _render_flashcard_tts(vocab: list, ders_no: int, color: str, color_light: str):
    ...
    meaning_html = f'<div class="meaning">{tr_text}</div>' if show_meaning else '<div class="meaning" style="opacity:0.5">Anlami gormek icin tiklayin</div>'
    ...
    <div style="background:linear-gradient(135deg,{color},{color_light});
        ...
        <div style="font-size:2rem;font-weight:800">{it_text}</div>
        ...
        <div style="display:flex;gap:10px;margin-top:12px;">
            <button onclick="speakIT('{it_escaped}', 0.85)"
                ...
                border-radius:24px;padding:6px 16px;cursor:pointer;font-size:0.9rem;">
                &#128266; Normal</button>
            <button onclick="speakIT('{it_escaped}', 0.55)"
        ...
        <div style="font-size:1.2rem;margin-top:12px;opacity:0.9">
            {'<span>' + tr_text + '</span>' if show_meaning else '<span style="opacity:0.5">Anlami gormek icin tiklayin</span>'}
        </div>
    ...
    """
    _components.html(flashcard_html, height=340)
    ...
    if examples:
        ...
        for ex in examples:
            ...
            <div style="background:white;border:1px solid #e2e8f0;border-radius:8px;
                 padding:10px 14px;border-left:3px solid #dc2626;
                 display:flex;justify-content:space-between;align-items:center;">
                <div style="flex:1;">
                    <div style="font-weight:600;color:#b91c1c">{it_text}</div>
                    <div style="color:#64748b;font-size:0.9rem;margin-top:2px">{tr_text}</div>
                </div>
                <button onclick="speakIT('{it_escaped}', 0.85)"
                    ...
                    box-shadow:0 2px 6px rgba(220,38,38,0.3);flex-shrink:0;margin-left:10px;"
                    title="Sesli Oku">&#128266;</button>
            </div>
            """

        examples_html += '</div>'
        height = len(examples) * 75 + 40
        _components.html(examples_html, height=min(height, 1500), scrolling=True)
...
def _render_reading(lesson: dict, ders_no: int):
    ...
    text_it = reading.get("it", "")
    text_tr = reading.get("tr", "")

    st.markdown(f"### {title}")

    # TTS ile okuma parcasi
    it_escaped = text_it.replace("'", "\\'").replace('"', '\\"').replace("\n", " ")
    ...
    <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:12px;
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
            ...
            <button onclick="speakIT('{it_escaped}', 0.8)"
                style="background:linear-gradient(135deg,#dc2626,#991b1b);color:white;
                border:none;border-radius:24px;padding:6px 16px;cursor:pointer;
                font-size:0.85rem;box-shadow:0 2px 6px rgba(220,38,38,0.3);
                display:flex;align-items:center;gap:4px;flex-shrink:0;"
                title="Parcayi Dinle">&#128266; Dinle</button>
        </div>
        <div style="color:#78350f;white-space:pre-line">{text_it}</div>
    </div>
    """
    _components.html(reading_html, height=max(200, len(text_it) // 2 + 120), scrolling=True)
...
def _render_exercises(lesson: dict, ders_no: int, progress: dict):
    ...
    for ex_idx, exercise in enumerate(exercises):
        ...
        <div class="fono-exercise-box">
            <div style="font-weight:700; color:#b91c1c; font-size:1rem; margin-bottom:4px">
                {ex_title}
            </div>
            <div style="color:#e11d48; font-size:0.85rem">
        ...
        for q_idx, q in enumerate(questions):
            ...
            elif ex_type == "translate_to_it":
                correct = _render_translate(q, q_key, is_checked, "it")
...
def _get_exercise_type_label(ex_type: str) -> str:
    labels = {
        ...
        "translate_to_tr": "Italyanca -> Turkce Ceviri",
        "translate_to_it": "Turkce -> Italyanca Ceviri",
...
def _render_translate(q: dict, key: str, checked: bool, target: str) -> bool:
    source = q.get("source", "")
    answer = q.get("answer", "")
    direction = "Turkce'ye cevirin" if target == "tr" else "Italyanca'ya cevirin"
...
def _check_answer(q: dict, key: str, ex_type: str) -> bool:
    ...
    elif ex_type in ("translate_to_tr", "translate_to_it"):
        valid = [q.get("answer", "")]
        if q.get("alt_answers"):
...
def _render_gunluk_pratik():
    ...
    if not data or not data.get("chapters"):
        st.warning("Gunluk Pratik Italyanca verileri yuklenemedi.")
        return

    chapters = data["chapters"]

    st.markdown("""
    <div style="background:linear-gradient(135deg,#7f1d1d 0%,#dc2626 50%,#be123c 100%);
         padding:24px 28px;border-radius:14px;margin-bottom:20px;color:white;">
        <h2 style="margin:0;font-size:1.5rem;font-weight:800">Gunluk Pratik Italyanca</h2>
        <p style="margin:4px 0 0 0;opacity:0.85;font-size:0.88rem">
            Pratik Italyanca-Turkce El Kitabi -- 13 Bolum, 70+ Alt Konu, 500+ Cumle & Sesli Okuma
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#fff1f2,#ffe4e6);border:1px solid #fda4af;
         border-radius:12px;padding:14px 18px;margin-bottom:16px;">
        <div style="font-weight:700;color:#b91c1c;font-size:1.1rem">{section['title']}</div>
        <div style="color:#e11d48;font-size:0.82rem;margin-top:2px">{len(phrases)} ifade</div>
    </div>
    """, unsafe_allow_html=True)
...
def _render_pratik_cards(phrases: list, sec_id: str):
    """Kart gorunumu - TTS destekli Italyanca."""
    cards_html = _TTS_JS_CORE + '<div style="display:flex;flex-direction:column;gap:10px;">'
    ...
    for i, p in enumerate(phrases):
        ...
        <div style="background:linear-gradient(135deg,#111827,#1A2035);padding:14px 18px;
             border-radius:12px;border:1px solid #fecaca;border-left:4px solid #dc2626;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                <div style="flex:1;">
                    <div style="font-weight:700;color:#b91c1c;font-size:1.05rem;margin-bottom:3px">
                        {it_text}
                    </div>
                    <div style="color:#e11d48;font-size:0.82rem;font-style:italic;margin-bottom:4px">
                        /{pron}/
                    </div>
...
def _render_pratik_list(phrases: list, sec_id: str):
    """Tablo gorunumu - TTS destekli Italyanca."""
    list_html = _TTS_JS_CORE + """
    <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
        <thead>
            <tr style="background:linear-gradient(135deg,#991b1b,#dc2626);color:white;">
                <th style="padding:10px 12px;text-align:left;border-radius:8px 0 0 0;">Turkce</th>
                <th style="padding:10px 12px;text-align:left;">Italiano</th>
    ...
    for i, p in enumerate(phrases):
        ...
        <tr style="background:{bg};border-bottom:1px solid #e2e8f0;">
            ...
            <td style="padding:8px 12px;color:#b91c1c;font-weight:700">{it_text}</td>
            <td style="padding:8px 12px;color:#e11d48;font-style:italic;font-size:0.82rem">/{pron}/</td>
            <td style="padding:8px 8px;text-align:center">
                <button onclick="speakIT('{it_escaped}', 0.85)"
                    style="background:#dc2626;color:white;border:none;border-radius:50%;
...
def _render_pratik_quiz(phrases: list, sec_id: str):
    """Mini quiz - Turkce'den Italyanca'ya ceviri."""
    if len(phrases) < 3:
    ...
    st.markdown("### Mini Quiz -- Turkce -> Italyanca")

    correct_count = 0
    for qi, idx in enumerate(indices):
        p = phrases[idx]
        st.markdown(f"**{qi+1}.** {p['tr']}")
        user_ans = st.text_input("Italyanca karsiligi:", key=f"{quiz_key}_{qi}",
                                  placeholder="Cevabinizi yazin...", label_visibility="collapsed")
        if is_checked:
            norm_user = user_ans.strip().lower().rstrip(".!?,;:").strip()
            norm_answer = p['it'].strip().lower().rstrip(".!?,;:").strip()
            if norm_user == norm_answer:
                st.success(f"Dogru! {p['it']}")
                correct_count += 1
            else:
                st.error(f"Dogru cevap: **{p['it']}** /{p.get('pron', '')}/")
...
def _render_vocab_bank(lessons: list):
    ...
    color = _BOOK_INFO[bid]["color"]
    color_light = _BOOK_INFO[bid]["color_light"]
    ...
    search = st.text_input("Kelime Ara:", key=f"{_PREFIX}vb_search", placeholder="Italyanca veya Turkce...")
    if search:
        all_words = [(w, d) for w, d in all_words
                     if search.lower() in w.get("it", "").lower()
    ...
    if all_words:
        bank_html = _TTS_JS_CORE + '<div style="display:flex;flex-direction:column;gap:10px;">'

        for i in range(0, len(all_words), 3):
            bank_html += '<div style="display:flex;gap:10px;flex-wrap:wrap;">'
            for j in range(3):
                idx = i + j
                ...
                <div style="flex:1;min-width:200px;background:linear-gradient(135deg,#111827,#1A2035);
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                        <div style="flex:1;">
                            <div style="font-weight:700;color:#b91c1c;font-size:1.1rem">{it_text}</div>
                            <div style="color:#e11d48;font-size:0.82rem;font-style:italic">/{pron}/</div>
                            <div style="color:#475569;font-size:0.92rem;margin-top:2px">{tr_text}</div>
                        ...
                        <button onclick="speakIT('{it_escaped}', 0.85)"
                            ...
                            box-shadow:0 2px 6px rgba(220,38,38,0.3);flex-shrink:0;margin-left:8px;"
                            title="Sesli Oku">&#128266;</button>
                    </div>
                </div>
                """
...
def render_fono_italyanca():
    ...
    if view == "lesson":
        ...
        tabs = st.tabs(["Dersler", "Ilerleme", "Kelime Bankasi", "Gunluk Pratik Italyanca"])
        with tabs[0]:
